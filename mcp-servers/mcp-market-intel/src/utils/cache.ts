...
export class CacheManager {
  ...
  static coordKey(lat: number, lng: number, prefix: string = ''): string {
    return `${prefix}:${Math.round(lat * 100) / 100},${Math.round(lng * 100) / 100}`;
  }
  static stateKey(state: string, prefix: string = ''): string {
    return `${prefix}:${state.toLowerCase().replace(/\s+/g, '-')}`;
  }
}
