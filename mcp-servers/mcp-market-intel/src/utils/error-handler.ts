...
export class ErrorHandler {
  ...
  handleToolError(error: unknown, toolName: string) {
    ...
    let userMessage = 'An unexpected error occurred while analyzing market data.';
    if (error instanceof Error) {
      const msg = error.message.toUpperCase();
      if (msg.includes('429') || msg.includes('RATE_LIMIT')) userMessage = 'Market data service is busy.';
      else if (msg.includes('TIMEOUT')) userMessage = 'Market data service timed out.';
      else if (msg.includes('INVALID_COORDINATES')) userMessage = 'Invalid coordinates.';
      else userMessage = `Market data error: ${error.message}`;
    }
    return {
